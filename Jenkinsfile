...
agent any
     stages{
        ...
        // stage('Build image'){
        //     steps{
        //         sh "docker build -t contact_crm-${branch}-b${BUILD_NUMBER} -f Dockerfile ."
        //     }
        // }
        // stage('run image')
        // {
        ...
        //         script{
        //            containerID = sh (
        //            script: "docker run -d -p 8000:8000 --name contact_crm-${branch}-V${BUILD_NUMBER} --network tg-net --env-file ./.env contact_crm-${branch}-b${BUILD_NUMBER}", 
        //            returnStdout: true
        //            ).trim()
